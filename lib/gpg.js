...
exports.run = async (runArgs) => {
    ...
    if (srcStream && !Utils.isStream(srcStream)) {
        ...
        if (srcStream.length <= FILE_NAME_LENGTH_LIMIT && await internals.fileExists(srcStream)) {
            ...
            srcStream = await Fs.createReadStream(srcStream);
        }
        else {
...
exports.keyExists = (keyIdentifier, keyType) => {
    ...
    return !!exports.utils.firstKeyForIdentifier(keyIdentifier, keyType);
};
...
exports.genKeys = async (args) => {
    ...
    // A setting to delete the tmp file on process exit
    ...
    const newKey = KeyConfig.get({ name: keyIdentifier, comment, email });

    await Fs.writeFile(tmpF.path, newKey);

    const optionalOpts = password ? [] : ['--batch'];

    try {
        const res = await exports.run([
            ...optionalOpts,
            '--gen-key',
            tmpF.path
        ]);
...
internals.getOutput = async (adapterProcess) => {
    ...
    const { waitForProcessExit } = internals;
    ...
    const exitCode = await waitForProcessExit(adapterProcess);

    if (exitCode !== 0) {
        throw new Error(`Exit code was not 0 it was ${exitCode}, output was ${out}`);
    }
...
internals.waitForProcessExit = async (processArg) => {

    const p = Utils.getPromise();

    processArg.on('exit', (code) => {
