...
exports.decrypt = async (src, destPath, password) => {
    ...
    if (!await internals.fileExists(src)) {
        return { err: new Error('At this time for \'decrypt\', src must be a valid filepath') };
    }
    ...
    if (destPath && await internals.fileExists(destPath)) {
...
internals.fileExists = async (path) => {
