...
const TestKeyInfo = require('@xdcreative/doggo/test/test-key-info');
...
module.exports = {
    ...
    genKeys: async (args) => {
        ...
        const [,fingerprint] = res.match(/.gnupg\/openpgp-revocs.d\/(.+).rev/);
    ...
    deleteKey: async (args) => {
        ...
        const { type, fingerprint, password } = args;

        const res = await Gpg.deleteKeys({
            type,
            fingerprint,
            password
        });
