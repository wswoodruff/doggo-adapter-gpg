...
exports.getOutput = async (adapterProcess) => {
    ...
    adapterProcess.on('exit', (code) => {
        ...
        if (knownErrMatchKey && !errOut) {
            errOut = module.exports.GPG_ERRORS[knownErrMatchKey];
        }
...
internals.checkOutForErr = (maybeErr) => {
    ...
    const [maybeFoundErr] = internals.objStrFind(module.exports.GPG_ERRORS, maybeErr) || [];
...
internals.checkForStdErrException = (maybeErr) => {
    ...
    // Just grab the first match
    const [maybeFoundException] = internals.objStrFind(module.exports.KNOWN_STDERR_EXCEPTIONS, maybeErr) || [];
